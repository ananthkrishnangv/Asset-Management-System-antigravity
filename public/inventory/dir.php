<?php
/**
 * Divisional Inventory Register (DIR) Management
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax'])) {
    ...
    if (isset($_POST['add'])) {
        ...
        try {
            $serialNumber = SerialNumber::generateForInventory('DIR');
            ...
            ActivityLog::log(
                ...
                $itemId,
                'dir',
                'Created DIR item: ' . $data['item_description'] . ' (' . $serialNumber . ')'
            );
    ...
    if (isset($_POST['update'])) {
        ...
        ActivityLog::log(
            ...
            $itemId,
            'dir',
            'Updated DIR item: ' . $data['item_description'],
            $oldData,
            $data
        );
    ...
    if (isset($_POST['delete_id'])) {
        ...
        if ($item) {
            ...
            ActivityLog::log(
                ...
                $itemId,
                'dir',
                'Deleted DIR item: ' . $item['item_description'] . ' (' . $item['serial_number'] . ')'
            );
    ...
    $total = $db->fetchValue(
        "SELECT COUNT(*) FROM inventory_items WHERE inventory_type = 'dir' AND is_active = 1
...
$pageTitle = 'Divisional Inventory Register (DIR)';
$pageSubtitle = 'Manage division-level inventory items';
